...
#
# 高级选项:
#   VERSION=v0.4.1 bash scripts/install.sh   # 指定版本
...
if [ "$(echo "$PY_VERSION < 3.10" | $PYTHON_BIN -c 'import sys; print(sys.version_info < (3,10))')" = "True" ]; then
...
$PYTHON_BIN -m pip install "vidknot @ git+$REPO_URL@$VERSION" 2>&1 | tail -3
