...
from ..douyin_api import DEFAULT_THIRD_PARTY_APIS
...
class DouyinPlatform(BasePlatform):
    ...
    def _download_with_retry(
        ...
        """下载视频直链（指数退避重试）。实现见 core/douyin_api.py。"""
    ...
    def _call_third_party_api(
        ...
        """调用单个第三方 API 获取视频直链（指数退避重试）。实现见 core/douyin_api.py。"""
        return douyin_api.call_third_party_api(url, api, max_retries=max_retries)
    ...
    def _parse_api_response(
        ...
        """按路径提取视频直链。实现见 core/douyin_api.py。"""
        return douyin_api.parse_api_response(data, response_path, api_name)
